...
use crate::select_gates::simple_dist_select::dist_select_simple;
...
// n: number of input qubits, m: interaction count, l: parallel parameter(high count)
#[pyfunction]
pub fn dist_select_m_body(n: i32, m: i32, l: i32) -> PyResult<String> {
    // prepare qubits
    let first_qubit = cellize(Qubit::new("first"));
    let inputs = (0..n)
        .map(|i| cellize(Qubit::new(&format!("input_{}", i))))
    ...
    // get controls
    let first_control_from = Qubit::control(first_qubit.clone());
    let (inputs, dist_select_ancillas, controls) = dist_select_simple(
        l,
        n - l,
        first_control_from,
        inputs,
        "dist_select_m_body".to_string(),
    );
    // inject data
    let applied_targets = combine_random_cnots_m_interaction(m, controls, targets);
    // collect qubits
    ...
    qubits.extend(inputs);
    qubits.extend(ancillas);
    qubits.extend(dist_select_ancillas);
    qubits.extend(applied_targets);
    // get qasm string
    let qasm_file = to_qasm(qubits);
    Ok(qasm_file.to_string())
